...
int main()
{
    ...
    Shader ourShader("./src/4.2_Stencil testing/shader/model.vert", "./src/4.2_Stencil testing/shader/model.frag");
    Shader rimShader("./src/4.2_Stencil testing/shader/model.vert", "./src/4.2_Stencil testing/shader/rim.frag");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 第二步：渲染稍微缩放的立方体，这次禁用模板写入
        // 因为模板缓冲区现在填满了几个1，缓冲区为1的部分不会被渲染
        // 大小的差异，看起来就像边框了
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
        glStencilMask(0x00);
